...
if (!empty($_COOKIE['simple_cart'])) {
    $encrypted_data = $_COOKIE['simple_cart'];
    $decrypted = openssl_decrypt(base64_decode($encrypted_data), 'AES-128-ECB', $secret_key);
    
    if ($decrypted) {
        $uncompressed = @gzuncompress($decrypted); // РАЗЖИМАЕМ
        if ($uncompressed) {
