...
function generate_products_table($products, $original_articles = null) {
    if (empty($products)) {
        ...
        $nonce = wp_create_nonce('component_request');
        ...
        <div class="component-request-form">
            ...
            <form method="POST" class="order-form">    
                ...
                <input type="hidden" name="component_nonce" value="' . esc_attr($nonce) . '">
    ...
    <div class="header-table">
        ...
        <div class="header-table-buttonWrapper">
            <button class="qutaBtn">Запросить квоту</button>
        </div>
    </div>
        ...
        <table class="product-search-table table-striped table-hover align-middle mb-0">
            <thead class="table-light">
                <tr>
                    ...
                    <th scope="col">Запросить квоту</th>
    ...
    foreach ($sorted_products as $product) {
        ...
        $quote_checkbox = '<div class="form-check-table"><input class="form-check-table-input quote-checkbox" type="checkbox" value="1"></div>';
        ...
        $clean_article = $product['article'];
        $clean_brand = $brand;
        $price_fixed = number_format((float)$price_rub, 2, '.', '');
        $signature_string = $name . '|' . $available . '|' . $clean_article . '|' . $clean_brand . '|' . $price_fixed . '|' . $donor . '|' . $term . '|' . $min_order;
        $row_uid = hash_hmac('sha256', $signature_string, $secret_key);
        
        $gen_out = "----------------------------------------------------\n";
        $gen_out .= "[$log_time] ГЕНЕРАЦИЯ СТРОКИ ТАБЛИЦЫ\n";
        $gen_out .= "Name:       $name\n";
        $gen_out .= "Article:    $clean_article\n";
        $gen_out .= "available:  $available\n";
        $gen_out .= "Brand:      $clean_brand\n";
        $gen_out .= "Price Rub:  $price_rub -> Fixed: $price_fixed\n";
        $gen_out .= "Donor:      $donor\n";
        $gen_out .= "Term:       $term\n";
        $gen_out .= "Min Order:  $min_order\n";
        $gen_out .= "String:     [$signature_string]\n";
        $gen_out .= "Result UID: $row_uid\n";
        
        if (!empty($log_file)) {
            file_put_contents($log_file, $gen_out, FILE_APPEND);
        }
        
        $session_data[$row_uid] = $price_fixed;
        ...
        $html .= sprintf(
            '<tr class="column_head">
                ...
                <td class="quantity" data-label="Кол-во">
                    <input type="number" class="quantity-input form-control form-control-sm"
                        value="%8$d" min="%9$d" max="%10$d" %11$s style="width: 70px !important;">
                ...
                <td class="quote-cell" data-label="Запросить квоту">%19$s</td>
                <td data-label="В корзину">
                    <button class="add-to-cart-btn btn btn-sm btn-success custom"
                            data-article="%16$s" data-name="%2$s" data-brand="%3$s"
                            data-price="%6$F" data-minq="%9$d" data-available="%10$d"
                            data-donor="%1$s" data-term="%18$s" data-uid="%20$s" %11$s> 
            ...
            /* 2  */ esc_html($name),
            /* 3  */ esc_html($brand),
            /* 4  */ esc_html($available),
            /* 5  */ esc_html($orderdays),
            /* 6  */ $price_rub,
            /* 7  */ $priceHtml,
            /* 8  */ (int) $quantity_input,
            /* 9  */ (int) $min_order,
            /* 10 */ (int) $available,
            /* 11 */ $disabled,
            /* 12 */ esc_html($folddivision_val),
            /* 13 */ esc_html($sPack),
            /* 14 */ $total_rub,
            /* 15 */ number_format($total_rub, 2, ',', ' '),
            /* 16 */ esc_html($product['article'] ?? $name),
            /* 17 */ esc_url($basket_icon),
            /* 18 */ esc_html($term),
            /* 19 */ $quote_checkbox,
            /* 20 */ $row_uid,
        );
    ...
    if (!empty($session_data)) {
        ...
        if (!isset($_SESSION['valid_product_uids'])) {
            $_SESSION['valid_product_uids'] = [];
        }
        $_SESSION['valid_product_uids'] = array_merge($_SESSION['valid_product_uids'], $session_data);
    }
